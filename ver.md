...
* **다운로드 속도 대폭 개선 (aria2c 연동)**
  * 유튜브의 다운로드 속도 제한(스로틀링)을 우회하기 위해 단일 커넥션 대신 `aria2c` 멀티스레드 다운로더를 도입했습니다.
  * 영상을 16개의 조각으로 동시 분할 다운로드(`-x 16 -s 16`)하여 다운로드 소요 시간을 획기적으로(초 단위) 단축시켰습니다.
  * 클라우드 배포용 `Dockerfile`에 `aria2` 패키지 설치 로직을 추가했습니다.
...
* `brew install aria2` 로컬 설치 및 `Dockerfile` 패키지 명세 반영 완료
* `app.py` `ydl_opts` 내 `external_downloader` 속성 정상 주입 확인
